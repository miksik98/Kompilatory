...
class TypeChecker(NodeVisitor):
    ...
    def visit_Statement(self, node):
        if isinstance(node, AST.Statements):
            ...
            elif isinstance(node, AST.MainStatements):
                return self.visit_MainStatements(node)
            elif isinstance(node, AST.PrintExpressions):
                return self.visit_PrintExpressions(node)
